...
for box in camera_IDs.keys():
    command += ' -p:Box{}.SerialNumber={}'.format(box, camera_IDs[box])
    if box in subjects.keys(): # Box is being used.
        command += ' -p:Box{}.Enable=True'.format(box)
    else: # Box not being used.
        command += ' -p:Box{}.Enable=False'.format(box)
    ...
    for i, (box, subject) in enumerate(subjects.items()):
        if not pipes_open[i]:
            ...
            if pipe.split('\\')[-1] in os.listdir(r'\\.\pipe'):
                ...
                ffmpeg_processes.append(Popen(r'ffmpeg -y -f rawvideo -vcodec rawvideo -s 1152x1024 -pix_fmt bayer_rggb8 -r 50 -i {} -c:v h264_nvenc -profile:v high -preset slow -an -f segment -segment_time 300 -reset_timestamps 1 {}'.format(pipe, video_file_path)))
    time.sleep(0.1)
